...
- `latex/academic_cv.tex`: the article-style LaTeX CV layout.
- `src/cv_config.toml`: display name, optional ORCID ID, ADS aliases, and Google Scholar link.
- `src/manual_publications.toml`: manual `submitted` entries that ADS cannot keep up to date for you.
- `src/generate_cv.py`: fetches ADS records, computes metrics, renders `latex/generated/publications.tex`, and builds `compiled/index.html`.
...
2. In `src/cv_config.toml`, confirm `display_name`, `name_aliases`, and `scholar_url`.
...
- `latex/generated/publications.tex`
- `compiled/index.html`
...
- If `ADS_DEV_KEY` is not set locally, the helper script still builds a fallback PDF using the placeholder text in `latex/academic_cv.tex`.
